...
$(document).on('click', '.project-delete', function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            // Удаляем задачу на клиентской стороне
            $('#' + projectId).remove();
            showModal('Проект удален'); 
        },
        error: function () {
            showModal('Ошибка удалении проекта'); 
        }
    });
});
...
$(document).on('change', '#update-role-team', function () {
    var selectedValue = $(this).find('option:selected').attr('id');
    var UserId = $(this).closest('li').attr('id');
    var type = $(this).data('type');
    $.ajax({
        url: '/Team/UpdateRoleTeamStaff',
        type: 'GET',
        data: { UserId: UserId, selectedValue: selectedValue ,type:type},
        success: function (response) {
            // Обработка успешного ответа
            console.log(response);
        },
...
$(document).on('click', '#urgency-task', function () {
    ...
    $select.append('<option id="1">важно, срочно</option>');
    $select.append('<option id="2">важно, не срочно</option>');
    $select.append('<option id="3">неважно, срочно</option>');
    $select.append('<option id="4">неважноЮ не срочно</option>');
    $itemUser.append($select);
});
$(document).on('click', '#start-project', function () {
    var $newProject = $(this).closest('.new-Project');
    var projectId = $newProject.attr('id');
    var end = $(this).data('end');
    $.ajax({
        url: '/Project/StartProject', // Замените на ваш URL
        type: 'GET',
        data: { projectId: projectId, end: end }, // Передаем id задачи в теле запроса
        success: function (response) {
            if (response.success) {
                var status = $newProject.find('.content-ID h4')
                status.text("Разработка"); 
                showModal('Запуск проекта'); 
            }
            else {
                showModal('Завершите настройку проекта');
            }
        },
        error: function (error) {
            showModal('Ошибка при запуске проекта');
        }
    });
});
        ...
        $(this).data('timeout', setTimeout(function () {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    showModal('Проект изменен'); 
                },
                error: function (error) {
                    console.error("Error sending data to controller:", error);
                    showModal('Ошибка при изменении проекта'); 
                }
            });
     ...
     $(document).on("change", "#taskId, #taskName, #taskDescription, #start-date-task, #end-date-task", function () {
        ...
        $(this).data('timeout', setTimeout(function () {
            $.ajax({
                ...
                success: function (response) {
                    console.log("Data sent to controller:", response);
                    showModal('Задача измененена'); 
                },
                error: function (error) {
                    console.error("Error sending data to controller:", error);
                    showModal('Ошибка при изменении задачи'); 
                }
            });
        }, 1000)); 
    });

    $(document).on("change", "#subtasksDescription", function () {
        ...
        $(this).data('timeout', setTimeout(function () {
            $.ajax({
                ...
                success: function (response) {
                    console.log("Data sent to controller subtask:", response);
                    showModal('Подзадача измененена'); 
                },
                error: function (error) {
                    console.error("Error sending data to controller:", error);
                    showModal('Ошибка при изменении подзадачи'); 
                }
            });
        }, 1000)); // Wait for 1 second after user stops typing
    });
...
/// MODAL WINDOW
$(document).ready(function () {
    $(document).on('click', '.btn-add-team, .add-departments,.container-team-ptoject a', function (e) {
        e.preventDefault();
...
$(document).on('click', '#select-btn-file', function () {
    $(this).closest('.options').find('.option-File').toggleClass("active");
}).on('mouseleave', '#select-btn-file, .option-File', function () {
    var $selectMenu = $('.option-File.active');
    setTimeout(function () {
        if (!$selectMenu.is(':hover')) {
            $selectMenu.removeClass("active");
        }
...
$(document).on('change', '#fileInput', function (e) {
    ...
    var $newTask = $(this).closest('.new-Task');
    var TaskId = $newTask.attr('id');
    formData.append('TaskId', TaskId); // Append TaskId to the FormData
    $.ajax({
        url: '/Project/UploadFile',
        type: 'POST',
        data: formData, // Send the FormData directly
        contentType: false, // The browser sets the correct content type
        processData: false, // Let jQuery handle the data
        success: function (result) {
            // Handle the result
        },
        error: function () {
            showModal('Ошибка сохранение файла'); 
        }
    });
});
$(document).on('click', '#btn-file-open', function (e) {
    e.preventDefault();
    var $newTask = $(this).closest('.new-Task');
    var TaskId = $newTask.attr('id');
    $.ajax({
        url: '/Project/OpenFile', // URL действия контроллера с параметром id в строке запроса
        type: 'GET',
        data: { TaskId: TaskId },
        success: function (result) {
            window.location.href = '/Project/OpenFile?taskId=' + TaskId;
        }
    });
});
$(document).on('change', '#fileInput-task', function (e) {
    var file = this.files[0];
    var formData = new FormData();
    formData.append('file', file);
    var $newTask = $(this).closest('.item-task');
...
$(document).on('click', '#btn-file-open-task', function (e) {
    e.preventDefault();
    var $newTask = $(this).closest('.item-task');
    var TaskId = $newTask.attr('id');
    $.ajax({
        url: '/Task/OpenFile', // URL действия контроллера с параметром id в строке запроса
        type: 'GET',
        data: { TaskId: TaskId },
        success: function (result) {
            window.location.href = '/Project/OpenFile?taskId=' + TaskId;
...
$(document).ready(function () {
    $(document).on('click', '#delete-department', function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                $('#tabContentTeam').html(data);
                showModal('Отдел удален'); 
            },
            error: function (xhr, status, error) {
                console.log(error);
                showModal('Ошибка удаления отдела'); 
            }
        });
    });
});
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    showModal('Команда удалена'); 
                }
          /*     *//* *//*$('#tabContentTeam').html(data);*/
            },
            error: function (xhr, status, error) {
                console.log(error);
                showModal('Ошибка удаления команды'); 
            }
        });
    });
});
...
function showModal(text) {
    ...
    $('.modalss').addClass('show');
    setTimeout(function () {
        $('.modalss').removeClass('show');
    }, 2000);
}
$(document).on('click', '.btn-closess', function () {
    $('#my-modalss').removeClass('show');
});
